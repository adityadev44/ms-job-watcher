"""slice's Kula ATS cache-once pagination, inline descriptions, and failure contracts."""
...
import slice_fetcher as fetcher
...
def raw_job(job_id, title, description="<p>Build things.</p>",
            offices=None, launch_at="2026-07-09T10:56:21.000Z"):
    return {
        "id": job_id,
        "title": title,
        "listed": True,
        ...
        "ats_job": {
            ...
            "offices": offices if offices is not None else [
                {"location": "Bengaluru, Karnataka, India", "country": "India",
                 "city": "Bengaluru", "state": "Karnataka", "remote": False}
            ],
        },
    }
...
def test_fetch_jobs_maps_fields_from_single_page(monkeypatch):
    get = Mock(return_value=api_response([raw_job(101, "SDE 2 - Backend")]))
    ...
    j = jobs[0]
    assert j['id'] == '101'
    assert j['title'] == 'SDE 2 - Backend'
    assert j['location'] == 'Bengaluru, Karnataka, India'
    assert j['posting_date'] == '2026-07-09'
    assert j['application_url'] == 'https://careers.kula.ai/slice/101'

    params = get.call_args.kwargs['params']
    assert params['accountName'] == 'slice'
...
def test_remote_anywhere_office_location_is_bare_india(monkeypatch):
    get = Mock(return_value=api_response([
        raw_job(1, "Agency Manager", offices=[{"location": "India", "remote": True}]),
    ]))
    monkeypatch.setattr(fetcher.requests, 'get', get)
...
def test_fetch_job_description_served_from_inline_cache(monkeypatch):
    get = Mock(return_value=api_response([
        raw_job(101, "SDE 2 - Backend", description="<p>Work with <b>LangChain</b> &amp; RAG.</p>"),
    ]))
    monkeypatch.setattr(fetcher.requests, 'get', get)

    description, posting_date = fetcher.fetch_job_description(
        'https://careers.kula.ai/slice/101'
    )
    assert description == 'Work with LangChain & RAG.'
    assert posting_date == '2026-07-09'
