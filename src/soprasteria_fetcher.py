"""
Sopra Steria job fetcher — SmartRecruiters public REST API.

Careers site: https://careers.soprasteria.com/india/ (India-specific page)
Company identifier confirmed live: "SopraSteria1"
(same SmartRecruiters platform family as Nagarro/Eurofins/NECSWS/PhonePe already
in this repo).

Search endpoint: GET /v1/companies/SopraSteria1/postings
  - `country` param (ISO-3166 lowercase code "in") is a reliable server-side
    filter — verified that country=in returns India-only results.
  - `q` (keyword) param is a genuine server-side filter — confirmed live:
    a nonsense token returns 0 results; ".net" returns relevant jobs only.
  - `limit` is capped at 100 server-side (consistent with other SmartRecruiters
    tenants). Pagination via `offset`.

Detail endpoint: GET /v1/companies/SopraSteria1/postings/<id>
  - `jobAd.sections.{jobDescription,qualifications,additionalInformation}.text`
    hold the HTML job content. `postingUrl` is the human-facing apply URL on
    jobs.smartrecruiters.com.

Live-verified 2026-09-05: 113 total India postings.
Keywords work server-side (0 results for nonsense, ~1 for ".NET").
"""
...
_HEADERS = {
    ...
    "Referer": "https://careers.soprasteria.com/india/",
}
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError(f"Sopra Steria: 429 rate-limited on attempt {attempt + 1}")
            r.raise_for_status()
            break
            ...
            if attempt == 2:
                raise RateLimitError(
                    f"Sopra Steria search failed after 3 attempts: {exc}"
                ) from exc
...
def fetch_job_description(application_url: str, timeout: int = 20) -> tuple[str, str]:
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError(f"Sopra Steria: 429 on detail for {job_id}")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
