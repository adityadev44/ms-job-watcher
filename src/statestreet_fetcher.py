"""Fetches State Street job listings via the Workday public REST API.

State Street's ATS is Workday, hosted at statestreet.wd1.myworkdayjobs.com
(tenant "statestreet", site "Global"). Confirmed live 2026-07-02:
POST to /wday/cxs/statestreet/Global/jobs returns real jobPostings.

State Street has ~200 India jobs (Bangalore, Hyderabad, Mumbai, Coimbatore).
Uses the capitalised "Location_Country" facet key (like Marsh McLennan),
not "locationCountry". Coimbatore (Tamil Nadu) is excluded via config.
"""
...
_BASE_URL = "https://statestreet.wd1.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/statestreet/Global/jobs"
_JOB_BASE = f"{_BASE_URL}/Global"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/statestreet/Global"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("State Street Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"State Street fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/Global/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("State Street description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"State Street description fetch failed: {exc}") from exc
