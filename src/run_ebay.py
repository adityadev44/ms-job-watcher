...
def run_ebay_pipeline(
    ...
    # eBay-only: require a core .NET/C#/ASP.NET term in the description,
    # not just any shared primary_skill (SQL Server/EF alone could easily
    # be a Java role).
    tech_terms = whole_cfg["ebay_search"].get("require_tech_in_description", [])
    if tech_terms:
        ...
        if desc_dropped:
            print("eBay description-tech filtered out (near-misses):")
