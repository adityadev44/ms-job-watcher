"""Fetches Target job listings via the Workday public REST API.

Target's ATS is Workday, hosted at target.wd5.myworkdayjobs.com
(tenant "target", site "targetcareers"). Confirmed live 2026-07-02:
POST to /wday/cxs/target/targetcareers/jobs returns real jobPostings.

Target's India Global Capability Centre in Bangalore. Uses the
capitalised "Location_Country" facet key (like MMC/State Street/Broadridge).
locationsText is usually "Bangalore,India" (no space after comma) but a few
postings show a full street address with no country word -- ", India"
...
_BASE_URL = "https://target.wd5.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/target/targetcareers/jobs"
_JOB_BASE = f"{_BASE_URL}/targetcareers"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/target/targetcareers"
...
_HEADERS = {
    ...
    "Referer": f"{_BASE_URL}/targetcareers",
}
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("Target Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"Target fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/targetcareers/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("Target description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"Target description fetch failed: {exc}") from exc
