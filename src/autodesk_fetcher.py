"""Fetches Autodesk job listings via the Workday public REST API.

Autodesk's ATS is Workday, hosted at autodesk.wd1.myworkdayjobs.com
(tenant "autodesk", site "Ext"). Confirmed live 2026-07-02:
POST to /wday/cxs/autodesk/Ext/jobs returns real jobPostings.

Autodesk's India engineering centres are in Bengaluru and Pune.
locationsText uses the abbreviated country code "IND" ("Bengaluru, IND"),
which does NOT contain the substring "india" -- ", India" appended
unconditionally is safe here since every IND-only string needs it.
"""
...
_BASE_URL = "https://autodesk.wd1.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/autodesk/Ext/jobs"
_JOB_BASE = f"{_BASE_URL}/Ext"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/autodesk/Ext"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("Autodesk Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"Autodesk fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("Autodesk description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"Autodesk description fetch failed: {exc}") from exc
