"""Fetch Equifax jobs from its official Workday careers tenant.
...
_BASE_URL = "https://equifax.wd5.myworkdayjobs.com"
_SITE = "External"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/equifax/{_SITE}/jobs"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/equifax/{_SITE}"
_PUBLIC_BASE = f"{_BASE_URL}/en-US/External"
_PAGE_SIZE = 20
_FACETS = {"locationCountry": ["c4f78be1a8f14da0ab49ce1162348a5e"]}
    ...
    for attempt in range(3):
        try:
            ...
            if response.status_code == 429:
                raise RateLimitError("Equifax Workday rate limited")
            ...
            if attempt == 2:
                raise RateLimitError(f"Equifax fetch failed: {exc}") from exc
...
def fetch_job_description(application_url: str, timeout: int = 20) -> tuple[str, str]:
    path = application_url.split(f"/en-US/External", 1)[-1]
