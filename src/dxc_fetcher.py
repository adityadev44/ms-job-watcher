"""Fetches DXC Technology job listings via the Workday public REST API.

DXC Technology's ATS is Workday, hosted at dxctechnology.wd1.myworkdayjobs.com
(tenant "dxctechnology", site "DXCJobs"). Confirmed live 2026-07-02:
POST to /wday/cxs/dxctechnology/DXCJobs/jobs returns real jobPostings.

DXC lists ~100+ India engineering jobs. locationsText uses the format
"IND - HR - GURGAON" (country - state - city codes) without the word
"India" -- appended client-side. Tamil Nadu appears as "- TN -", so the
config exclude list carries a "- TN -" entry in addition to "Chennai".
"""
...
_BASE_URL = "https://dxctechnology.wd1.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/dxctechnology/DXCJobs/jobs"
_JOB_BASE = f"{_BASE_URL}/DXCJobs"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/dxctechnology/DXCJobs"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("DXC Technology Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"DXC Technology fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/DXCJobs/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("DXC Technology description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"DXC Technology description fetch failed: {exc}") from exc
