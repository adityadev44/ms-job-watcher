"""Tower Research Capital jobs from its official Greenhouse board."""
from __future__ import annotations
import html, re, time
import requests
from urllib.parse import parse_qs, urlparse
_TOKEN="towerresearchcapital";_URL=f"https://boards-api.greenhouse.io/v1/boards/{_TOKEN}/jobs";_HEADERS={"User-Agent":"Mozilla/5.0","Accept":"application/json"}
_INDIA=("india","gurgaon","gurugram","bengaluru","bangalore","mumbai","hyderabad","delhi","noida","pune","chennai")
...
def _fill(timeout):
 ...
 for attempt in range(3):
  try:
   ...
   if attempt==2:raise RateLimitError(f"Tower Research Greenhouse fetch failed: {exc}") from exc
   time.sleep(2**attempt)
 rows=[]
 for x in r.json().get("jobs",[]):
  jid,title=str(x.get("id") or ""),(x.get("title") or "").strip()
  if not jid or not title:continue
  loc=((x.get("location") or {}).get("name") or "").strip();low=loc.lower()
...
def fetch_job_description(application_url,timeout=20):
 _fill(timeout);parsed=urlparse(application_url);jid=(parse_qs(parsed.query).get("gh_jid") or [parsed.path.rstrip("/").split("/")[-1]])[0]
 if jid not in _descriptions:jid=next((j["id"] for j in _jobs if j["application_url"]==application_url),jid)
 d=next((j["posting_date"] for j in _jobs if j["id"]==jid),"");return _text(_descriptions.get(jid,"")),d
