"""
Dell Technologies job fetcher — Oracle HCM Cloud Candidate Experience REST API.

Plain HTTP requests, no Playwright needed.
India jobs filtered server-side via location facet ID 300000000471053.
"""
...
_BASE_URL = "https://iawmqy.fa.ocs.oraclecloud.com"
...
_JOB_BASE = f"{_BASE_URL}/hcmUI/CandidateExperience/en/sites/careers/job"

_SITE_NUMBER = "CX_1001"
_INDIA_LOCATION_FACET_ID = 300000000471053
_PAGE_SIZE = 25
...
_HEADERS = {
    ...
    "ora-irc-language": "US",
    "Referer": f"{_BASE_URL}/hcmUI/CandidateExperience/en/sites/careers/jobs",
}
...
def fetch_jobs(
    ...
    *,
    ...
    # Dell's Oracle HCM CE finder string format (discovered via live network capture)
    finder = (
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                raise RateLimitError(f"Dell search failed after 3 attempts: {exc}") from exc
    ...
    for j in req_list:
        ...
        # Safety: skip non-India (should not happen with facet filter, but belt-and-suspenders)
...
def fetch_job_description(application_url: str, timeout: int = 20) -> tuple[str, str]:
    # Job ID is the last path segment of the application URL
