"""Fetches Ameriprise job listings via the Workday public REST API.

Ameriprise's ATS is Workday, hosted at ameriprise.wd5.myworkdayjobs.com
(tenant "ameriprise", site "ameriprise"). Confirmed live 2026-07-02:
POST to /wday/cxs/ameriprise/ameriprise/jobs returns real jobPostings.

Ameriprise Financial's India centres are in Hyderabad, Noida and Gurugram.
locationsText normally already ends in ", India".
"""
...
_BASE_URL = "https://ameriprise.wd5.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/ameriprise/ameriprise/jobs"
_JOB_BASE = f"{_BASE_URL}/ameriprise"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/ameriprise/ameriprise"
...
_HEADERS = {
    ...
    "Referer": f"{_BASE_URL}/ameriprise",
}
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("Ameriprise Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"Ameriprise fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/ameriprise/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("Ameriprise description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"Ameriprise description fetch failed: {exc}") from exc
