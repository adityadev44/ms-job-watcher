"""Fetches Rapido job listings via the Darwinbox candidate portal.

Company: Rapido Bike Taxi and Services Pvt Ltd ("Rapido"), India's leading
bike-taxi and auto-rickshaw platform. Headquartered in Bengaluru with
operations across major Indian cities. Has a growing engineering team focused
on mobility, logistics, and financial services.

ATS discovery (2026-09-26):
  www.rapido.bike/Careers links directly to the Darwinbox candidate portal at
  https://rapido.darwinbox.in/ms/candidate/careers — the older "candidate"
  SPA product tier, NOT the newer "candidatev2" front-end used by Airtel /
  Darwinbox's own board in this repo.

Cloudflare gating (confirmed 2026-09-26): plain requests.get() (even with a
realistic browser User-Agent) to any endpoint under rapido.darwinbox.in
...
Application URL: https://rapido.darwinbox.in/ms/candidate/careers/job/{id}
(inferred from the SPA's own routing conventions for the "candidate" tier).

India offices: Bengaluru (HQ), Hyderabad.
"""
...
def _location_from_job(job: dict) -> str:
    ...
    tenant returns (Rapido operates only in India).
    """
...
def _ensure_live_context(timeout: int = 30) -> None:
    ...
    from the same origin (rapido.darwinbox.in) to avoid CORS preflight failures
    ...
    for attempt in range(3):
        try:
            ...
            if result["status"] == 429:
                raise RateLimitError(f"Rapido: 429 rate-limited from {url}")
            if result["status"] != 200:
                raise RateLimitError(
                    f"Rapido API {url!r} returned HTTP {result['status']}"
                )
            ...
            if attempt < 2:
                ...
                continue
            raise RateLimitError(
                f"Rapido API call failed after 3 attempts: {exc}"
            ) from exc
    raise RateLimitError(f"Rapido: no response — {last_exc}")
...
def _fill_cache(timeout: int = 30) -> None:
    """Paginate through the entire Rapido Darwinbox board once and cache it.
    ...
    print(f"[Rapido] Cache filled: {len(collected)} total jobs")
...
def fetch_jobs(
    ...
    *,
    ...
    """Return a page of Rapido (Darwinbox) postings.
...
def fetch_job_description(application_url: str, timeout: int = 20) -> tuple[str, str]:
    """Return (description, posting_date) for a single Rapido job.
