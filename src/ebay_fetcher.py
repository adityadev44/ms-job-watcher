"""Fetches eBay job listings via the Workday public REST API.

eBay's ATS is Workday, hosted at ebay.wd5.myworkdayjobs.com
(tenant "ebay", site "apply"). Confirmed live 2026-07-02:
POST to /wday/cxs/ebay/apply/jobs returns real jobPostings.

eBay's India engineering centre is in Bengaluru. Uses the capitalised
"Location_Country" facet key. locationsText already contains "India".
"""
...
_BASE_URL = "https://ebay.wd5.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/ebay/apply/jobs"
_JOB_BASE = f"{_BASE_URL}/apply"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/ebay/apply"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("eBay Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"eBay fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/apply/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("eBay description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"eBay description fetch failed: {exc}") from exc
