...
def run_citi_pipeline(
    ...
    # Citi-only: require a core .NET/C#/ASP.NET term in the description,
    # not just any shared primary_skill (SQL Server/EF alone could easily
    # be a Java role).
    tech_terms = whole_cfg["citi_search"].get("require_tech_in_description", [])
    if tech_terms:
        ...
        if desc_dropped:
            print("Citi description-tech filtered out (near-misses):")
