"""Fetch Prudential jobs from its official Workday careers tenant.
...
_BASE_URL = "https://prudential.wd3.myworkdayjobs.com"
_SITE = "prudential"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/prudential/{_SITE}/jobs"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/prudential/{_SITE}"
_PUBLIC_BASE = f"{_BASE_URL}/en-US/prudential"
_PAGE_SIZE = 20
    ...
    for attempt in range(3):
        try:
            ...
            if response.status_code == 429:
                raise RateLimitError("Prudential Workday rate limited")
            ...
            if attempt == 2:
                raise RateLimitError(f"Prudential fetch failed: {exc}") from exc
...
def fetch_jobs(keyword: str, location: str, *, num: int = 20, start: int = 0, sort_by: str = "date", timeout: int = 20) -> list[dict[str, str]]:
    response = _request("POST", _SEARCH_URL, timeout=timeout, json={"appliedFacets": {}, "limit": min(num, _PAGE_SIZE), "offset": start, "searchText": keyword})
    jobs = []
...
def fetch_job_description(application_url: str, timeout: int = 20) -> tuple[str, str]:
    path = application_url.split(f"/en-US/prudential", 1)[-1]
