"""Fetches Adobe job listings via the Workday public REST API.

Adobe's ATS is Workday, hosted at adobe.wd5.myworkdayjobs.com
(tenant "adobe", site "external_experienced"). Confirmed live 2026-07-02:
POST to /wday/cxs/adobe/external_experienced/jobs returns real jobPostings.

Adobe's India engineering centres are in Bangalore and Noida.
`locationCountry` is not listed in the tenant's advertised facets response
but still works when applied directly (same quirk seen on WTW/Fiserv).
locationsText is city-only ("Bangalore", "Noida") -- ", India" appended.
"""
...
_BASE_URL = "https://adobe.wd5.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/adobe/external_experienced/jobs"
_JOB_BASE = f"{_BASE_URL}/external_experienced"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/adobe/external_experienced"
...
_HEADERS = {
    ...
    "Referer": f"{_BASE_URL}/external_experienced",
}
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("Adobe Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"Adobe fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/external_experienced/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("Adobe description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"Adobe description fetch failed: {exc}") from exc
