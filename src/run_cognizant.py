...
def run_cognizant_pipeline(
    ...
    # Cognizant-only: require a core .NET/C#/ASP.NET term in the
    # description, not just any shared primary_skill. Cognizant posts
    # thousands of India jobs with generic titles like "Senior Developer"
    # that rarely mention the actual tech stack. Requiring explicit
    # .NET/C# in the description keeps precision high.
    tech_terms = whole_cfg["cognizant_search"].get("require_tech_in_description", [])
    if tech_terms:
        ...
        if desc_dropped:
            print("Cognizant description-tech filtered out (near-misses):")
