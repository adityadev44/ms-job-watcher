"""Fetches Broadridge job listings via the Workday public REST API.

Broadridge's ATS is Workday, hosted at broadridge.wd5.myworkdayjobs.com
(tenant "broadridge", site "Careers"). Confirmed live 2026-07-02:
POST to /wday/cxs/broadridge/Careers/jobs returns real jobPostings.

Broadridge Financial Solutions (heavily .NET) lists ~30 India jobs, mostly
Bengaluru + Hyderabad. Uses the capitalised "Location_Country" facet key.
locationsText is office-name only ("Bengaluru-EPIP Industrial Area") --
", India" appended client-side.
"""
...
_BASE_URL = "https://broadridge.wd5.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/broadridge/Careers/jobs"
_JOB_BASE = f"{_BASE_URL}/Careers"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/broadridge/Careers"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("Broadridge Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"Broadridge fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("Broadridge description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"Broadridge description fetch failed: {exc}") from exc
