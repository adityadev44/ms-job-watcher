"""Fetches Kyndryl job listings via the Workday public REST API.

Kyndryl's ATS is Workday, hosted at kyndryl.wd5.myworkdayjobs.com
(tenant "kyndryl", site "KyndrylProfessionalCareers"). Confirmed live 2026-07-02:
POST to /wday/cxs/kyndryl/KyndrylProfessionalCareers/jobs returns real jobPostings.

Kyndryl (IBM's infrastructure-services spin-off) lists ~290 India software
jobs. Multi-site rollups show "2 Locations" -- ", India" appended client-side.
"""
...
_BASE_URL = "https://kyndryl.wd5.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/kyndryl/KyndrylProfessionalCareers/jobs"
_JOB_BASE = f"{_BASE_URL}/KyndrylProfessionalCareers"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/kyndryl/KyndrylProfessionalCareers"
...
_HEADERS = {
    ...
    "Referer": f"{_BASE_URL}/KyndrylProfessionalCareers",
}
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("Kyndryl Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"Kyndryl fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/KyndrylProfessionalCareers/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("Kyndryl description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"Kyndryl description fetch failed: {exc}") from exc
