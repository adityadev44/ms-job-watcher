"""
eClerx job fetcher — Oracle HCM Cloud Candidate Experience REST API.

Tenant: fa-ewji-saasfaprod1.fa.ocs.oraclecloud.com  |  Site: CX_1
Same REST pattern as Chubb/Hexaware/Icertis/WTW — plain HTTP requests, no
Playwright needed. Fetches globally and filters by "india" in PrimaryLocation
(no location facet ID used, same conservative approach as Hexaware/Icertis/WTW).
"""
...
_BASE_URL = "https://fa-ewji-saasfaprod1.fa.ocs.oraclecloud.com"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("eClerx: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"eClerx search failed: {exc}") from exc

    if r is None:
        raise RateLimitError(f"eClerx fetch: no response — {last_exc}")
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    time.sleep(2 ** attempt)
                    continue
                raise RateLimitError("eClerx description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"eClerx description fetch failed: {exc}") from exc

    if r is None:
        raise RateLimitError(f"eClerx description fetch: no response — {last_exc}")
