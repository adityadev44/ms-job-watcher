"""
WTW (Willis Towers Watson) job fetcher — Oracle HCM Cloud CE REST API.

Plain HTTP requests, no Playwright needed. Same API pattern as chubb_fetcher
but different tenant (eedu.fa.em3.oraclecloud.com) and site (CX_1003).

WTW has a small global job count, so all results are returned without an
India location facet and is_india_job() in matcher.py does the filtering.
"""
...
_BASE_URL = "https://eedu.fa.em3.oraclecloud.com"
...
_JOB_BASE = f"{_BASE_URL}/hcmUI/CandidateExperience/en/sites/CX_1003/job"

_SITE_NUMBER = "CX_1003"
_PAGE_SIZE = 25
...
_HEADERS = {
    ...
    "ora-irc-language": "US",
    "Referer": f"{_BASE_URL}/hcmUI/CandidateExperience/en/sites/CX_1003/jobs",
}
    ...
    *,
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                raise RateLimitError(f"WTW search failed after 3 attempts: {exc}") from exc
