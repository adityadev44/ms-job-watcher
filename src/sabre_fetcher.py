"""Fetches Sabre job listings via the Workday public REST API.

Sabre's ATS is Workday, hosted at sabre.wd1.myworkdayjobs.com
(tenant "sabre", site "SabreJobs"). Confirmed live 2026-07-02:
POST to /wday/cxs/sabre/SabreJobs/jobs returns real jobPostings.

Sabre (travel-technology / GDS) has a Bengaluru engineering centre.
...
_BASE_URL = "https://sabre.wd1.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/sabre/SabreJobs/jobs"
_JOB_BASE = f"{_BASE_URL}/SabreJobs"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/sabre/SabreJobs"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("Sabre Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"Sabre fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    if _JOB_BASE in application_url:
        ...
        ext_path = "/" + application_url.split("/SabreJobs/", 1)[-1]
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("Sabre description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"Sabre description fetch failed: {exc}") from exc
