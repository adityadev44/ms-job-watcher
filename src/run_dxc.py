...
def run_dxc_pipeline(
    ...
    # DXC-only: require a core .NET/C#/ASP.NET term to appear in the
    # description itself, not just any shared primary_skill (SQL Server/EF
    # alone could easily be a Java role). Same pattern as HCLTech's Layer 4.
    tech_terms = whole_cfg["dxc_search"].get("require_tech_in_description", [])
    if tech_terms:
        ...
        if desc_dropped:
            print("DXC Technology description-tech filtered out (near-misses):")
