"""
DTCC (Depository Trust & Clearing Corporation) job fetcher — Oracle HCM Cloud
Candidate Experience REST API.

Tenant: ebxr.fa.us2.oraclecloud.com  |  Site: CX_1
Same REST pattern as Chubb/Amex/JPMorgan/Icertis/Hexaware/eClerx/BNY — plain
HTTP requests, no Playwright needed. No India location facet ID discovered
(the LOCATIONS facet returned by the search response has no India entry to
select), so this fetcher fetches globally per keyword and filters India
client-side via "india" in PrimaryLocation, same conservative approach as
Icertis/WTW/Hexaware.
"""
...
_BASE_URL = "https://ebxr.fa.us2.oraclecloud.com"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("DTCC: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"DTCC search failed: {exc}") from exc

    if r is None:
        raise RateLimitError(f"DTCC fetch: no response — {last_exc}")
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    time.sleep(2 ** attempt)
                    continue
                raise RateLimitError("DTCC description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"DTCC description fetch failed: {exc}") from exc

    if r is None:
        raise RateLimitError(f"DTCC description fetch: no response — {last_exc}")
