"""Fetches Ather Energy job listings via the SmartRecruiters public REST API.

Company: Ather Energy Pvt Ltd — India's EV two-wheeler startup (recently IPO'd).
ATS: SmartRecruiters
Company identifier: "AtherEnergy" (verified live 2026-09-26 via API)
Careers site: https://careers.smartrecruiters.com/AtherEnergy

Discovery notes (2026-09-26):
  - SmartRecruiters confirmed: api.smartrecruiters.com/v1/companies/AtherEnergy/postings
    returns HTTP 200 with valid pagination structure (totalFound: 0 at time of check).
  - Ather Energy has software/firmware engineering teams in Bengaluru (HQ) and Chennai.
  - The board has 0 open positions as of check date — 0 matches is expected and
    not a fetcher defect. Ather recently IPO'd and hiring patterns may shift.
  - www.atherenergy.com/careers → HTTP 403 Forbidden (blocks headless requests).
  - careers.atherenergy.com    → HTTP 403 Forbidden (same WAF block).
  - Lever api.lever.co/v0/postings/atherenergy → HTTP 404 (no Lever board).
  - Greenhouse boards-api.greenhouse.io/v1/boards/atherenergy/jobs → HTTP 404.
  - jobs.ashbyhq.com/ather-energy → returns a "Jobs" heading with no listings;
    Ashby API also returns 404 — likely a stale/defunct Ashby account.
  - SmartRecruiters is the active, confirmed ATS.

API shape (same as Nagarro/PhonePe/NEC/Delhivery):
  GET https://api.smartrecruiters.com/v1/companies/AtherEnergy/postings
  Params:
    - country=in  → reliable server-side India filter
    - q=<keyword> → loose pre-filter only; shared matcher does real work
    - limit       → capped at 100 server-side
    - offset      → for pagination
  Response: {"content": [...], "totalFound": N, "limit": N, "offset": N}

Detail endpoint:
  GET https://api.smartrecruiters.com/v1/companies/AtherEnergy/postings/{id}
...
def _job_id_from_url(application_url: str) -> str:
    """'https://jobs.smartrecruiters.com/AtherEnergy/744...-slug' -> '744...'."""
...
def fetch_jobs(
    ...
    *,
    ...
    """Return a page of Ather Energy India jobs from SmartRecruiters.
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError(f"Ather Energy SmartRecruiters: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            raise RateLimitError(
                f"Ather Energy search failed after 3 attempts: {exc}"
            ) from exc

    if r is None:
        raise RateLimitError(f"Ather Energy search: no response — {last_exc}")
...
def fetch_job_description(
    ...
    """Return (description, posting_date) for one Ather Energy job.
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError(f"Ather Energy detail 429: {job_id}")
