"""Fetches LSEG job listings via the Workday public REST API.

LSEG's ATS is Workday, hosted at lseg.wd3.myworkdayjobs.com
(tenant "lseg", site "careers"). Confirmed live 2026-07-02:
POST to /wday/cxs/lseg/careers/jobs returns real jobPostings.

LSEG (London Stock Exchange Group) has a large Bengaluru engineering centre
with many .NET/C# roles. locationsText uses office codes like
"IND-BLR-Divyasree Technopolis" without the word "India" -- appended client-side.
"""
...
_BASE_URL = "https://lseg.wd3.myworkdayjobs.com"
_SEARCH_URL = f"{_BASE_URL}/wday/cxs/lseg/careers/jobs"
_JOB_BASE = f"{_BASE_URL}/careers"
_DETAIL_BASE = f"{_BASE_URL}/wday/cxs/lseg/careers"
        ...
        try:
            ...
            if r.status_code == 429:
                if attempt < 2:
                    ...
                    continue
                raise RateLimitError("LSEG Workday: 429 rate-limited")
            r.raise_for_status()
            break
            ...
            if attempt < 2:
                time.sleep(2 ** attempt)
                continue
            raise RateLimitError(f"LSEG fetch failed: {exc}") from exc
...
def fetch_job_description(
    ...
    for attempt in range(2):
        try:
            ...
            if r.status_code == 429:
                raise RateLimitError("LSEG description: 429 rate-limited")
            r.raise_for_status()
            break
        except RateLimitError:
            raise
            ...
            if attempt == 0:
                time.sleep(1)
                continue
            raise RateLimitError(f"LSEG description fetch failed: {exc}") from exc
