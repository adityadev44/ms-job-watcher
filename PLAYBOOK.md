...
- **Honeywell, Tech Mahindra** — false alarms. Both use Playwright and failed with a "Playwright Sync API inside the asyncio loop" error only when run back-to-back with ~116 other fetchers in one long-lived script; both work perfectly in a fresh process. Not a real bug — an artifact of the health-check script's own sequential execution, not of production (`run_all.py` runs each company as an independent thread via `ThreadPoolExecutor`, not sharing this kind of state).
